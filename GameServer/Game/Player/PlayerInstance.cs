...
using EggLink.DanhengServer.GameServer.Game.FightActivity; // 必须添加这一行
...
public partial class PlayerInstance(PlayerData data)
{
	...
	public FightActivityManager? FightActivityManager { get; private set; }
    ...
    private async ValueTask InitialPlayerManager()
    {
		...
		FightActivityManager = new FightActivityManager(this);
    ...
    if (Data.UnlockedRecipes.Count > 0)
    {
        ...
        {
            await ExpeditionManager.Initialize(); 
        }
